...
ssl_init(void)
{
	...
	if ((fd = open("/dev/urandom", O_RDONLY)) == -1) {
		log_message(ERROR_LOG, "Error opening /dev/urandom for reading: %s",
		               strerror(errno));
		return -1;
	}
	while (!RAND_status()) {
		if (read(fd, buf, sizeof(buf)) == -1) {
			log_message(ERROR_LOG, "Error reading from /dev/urandom: %s",
			               strerror(errno));
			close(fd);
			return -1;
		}
	...
	close(fd);
	if (!RAND_poll()) {
		log_message(ERROR_LOG, "RAND_poll() failed.");
		return -1;
	}
#else /* PURIFY */
	log_message(ERROR_LOG, "Warning: not seeding OpenSSL RAND due to PURITY!");
...
ssl_ssl_state_to_str(SSL *ssl)
{
	...
	ret = asprintf(&str, "%08x = %s%s%s%04x = %s (%s) [%s]",
	         ssl->state,
...
ssl_tmp_dh_callback(UNUSED SSL *s, int is_export, int keylength)
{
	...
	if (!(dh = DH_new())) {
		log_message(ERROR_LOG, "DH_new() failed.");
		return NULL;
	}
	switch (keylength) {
		case 512:
		...
		case 4096:
			...
			break;
		default:
			log_message(ERROR_LOG, "Unhandled DH keylength %i%s",
			               keylength,
			               (is_export ? " (export)" : ""));
			DH_free(dh);
			return NULL;
	...
	if (!dh->p || !dh->g) {
		log_message(ERROR_LOG, "Failed to load DH p and g from memory.");
		DH_free(dh);
		return NULL;
	}
               ...
               const char *extraname, EVP_PKEY *key)
{
	...
	if (ssl_x509_v3ext_add(&ctx, crt, (char *)"basicConstraints",
	                                  (char *)"CA:FALSE") == -1 ||
	    ssl_x509_v3ext_add(&ctx, crt, (char *)"keyUsage",
	                                  (char *)"digitalSignature,"
	                                  "keyEncipherment") == -1 ||
	    ssl_x509_v3ext_add(&ctx, crt, (char *)"extendedKeyUsage",
	                                  (char *)"serverAuth") == -1 ||
	    ssl_x509_v3ext_add(&ctx, crt, (char *)"subjectKeyIdentifier",
	                                  (char *)"hash") == -1 ||
	    ssl_x509_v3ext_add(&ctx, crt, (char *)"authorityKeyIdentifier",
	...
	if (!extraname) {
		...
		if (!names) {
			...
			if (asprintf(&cfval, "DNS:%s", extraname) == -1)
				goto errout;
			if (ssl_x509_v3ext_add(&ctx, crt, (char *)"subjectAltName",
			                       cfval) == -1) {
				free(cfval);
				goto errout;
			}
...
#define DBG_printf(...) 
	...
	ssize_t msglen, sidlen, suiteslen, compslen, tlsextslen;
			...
			case 0:
			{
				...
				while (extn > 0) {
					...
					if (snlen > TLSEXT_MAXLEN_host_name)
						goto out;
					if (sntype == 0) {
						servername = (char *)malloc(snlen + 1);
